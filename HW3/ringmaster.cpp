...
int main(int argc, char *argv[]){
    ...
    for(int i = 0; i<num_players;i++){
        std::string playerOwnIp;
        int playerfd = serverAccept(masterfd, &playerOwnIp);
        players[i] = playerfd;
        playerIp.push_back(playerOwnIp);
        send(playerfd,&i,sizeof(i),0);
        send(playerfd,&num_players,sizeof(num_players),0);
        int connected;
        recv(playerfd,&connected,sizeof(connected),MSG_WAITALL);
        printf("Player %d is ready to play\n",i);
    }
    //send players information right neighbor's information & numplayers
    for(int i = 0;i<num_players-1;i++){
        char playerNeighborIp[30];
        memset(playerNeighborIp, 0, sizeof(playerNeighborIp));
        strcpy(playerNeighborIp,playerIp[i+1].c_str());
        // std::cout<< "playerNeighborIp"<<playerNeighborIp<<std::endl;
        send(players[i],&playerNeighborIp,sizeof(playerNeighborIp),0);
    }
    char playerNeighborIp[30];
    memset(playerNeighborIp, 0, sizeof(playerNeighborIp));
    strcpy(playerNeighborIp,playerIp[0].c_str());
    // std::cout<< "playerNeighborIp"<<playerNeighborIp<<std::endl;
    send(players[num_players-1],&playerNeighborIp,sizeof(playerNeighborIp),0);
