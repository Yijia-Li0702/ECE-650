...
void * ts_malloc_nolock(size_t size){
  if(memHead_tls == NULL){
    block * mcb = getNewMem_tls(size+sizeof(block));
    if(mcb == NULL){
      return NULL;
    }
    return (char*)mcb + sizeof(block);
  ...
  while(curr != NULL){
    ...
    if(curr->size == size && curr->available){
      curr->available = 0;
      curr = remove_from_freelist_tls(curr);
      best = curr;
      return (char *)curr + sizeof(block);
    } else if(curr->size >= size + sizeof(block) && curr->available){
      //record the available one with the minimum size
...
block * remove_from_freelist_tls(block* curr){
  if(memHead_tls == curr||curr->prev == NULL){
    memHead_tls = curr->next;
  } else{
    curr->prev->next = curr->next;
  }
  if(curr->next != NULL){
...
block * split_blk_tls(block * curr, size_t size){
  ...
  new_mcb->prev = curr->prev;
  //put the new block into freelist
  if(curr == memHead_tls||curr->prev == NULL){
    memHead_tls = new_mcb;
  } else{
    curr->prev->next = new_mcb;
  }
